...
import { ButtonWrapper, Counter, ResultText, ResultTitle } from "./styles";
    ...
    <>
      ...
      {!isLoading && !result && (
        <Question
          ...
          selected={currentAnswerSelected}
      ...
      {result && (
        <>
          ...
          <ResultText>{resultExplanation[result]}</ResultText>
        </>
      )}

      <ButtonWrapper isRedoButton={!!result}>
        <Button onClick={!result ? handleNextQuestion : handleRedo}>
          {!result ? (isLastQuestion ? "End" : "Next") : "Retake"}
        </Button>
      </ButtonWrapper>
    </>
  );
